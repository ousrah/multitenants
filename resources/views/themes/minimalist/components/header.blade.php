...
<header class="sticky top-0 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm z-10 border-b border-slate-200 dark:border-slate-700">
    <div class="flex items-center justify-between h-16 px-4 sm:px-6 lg:px-8">
        ...
        <!-- Icônes de droite -->
        <div class="flex items-center space-x-2 rtl:space-x-reverse"> {{-- <-- AJOUT 1: Espacement inversé --}}

            <!-- Language Switcher -->
            <x-theme::language-switcher />

            <!-- Notifications (Exemple simple de drawer) -->
            <div x-data="{ open: false }" class="relative">
                <button @click="open = !open" class="p-2 rounded-full hover:bg-slate-200 dark:hover:bg-slate-700">
                ...
                <div x-show="open" @click.away="open = false"
                     class="absolute top-full right-0 mt-2 w-80 bg-white dark:bg-slate-800 rounded-lg shadow-lg border dark:border-slate-700
                            rtl:right-auto rtl:left-0" {{-- <-- AJOUT 2: Positionnement du drawer --}}
                     x-transition>
            ...
            <!-- Dark Mode Toggle -->
            <button @click="toggleDarkMode()" class="p-2 rounded-full hover:bg-slate-200 dark:hover:bg-slate-700">
            ...
            <!-- Profil (Exemple simple de drawer) -->
            <div x-data="{ open: false }" class="relative">
                ...
                <div x-show="open" @click.away="open = false"
                     class="absolute top-full right-0 mt-2 w-48 bg-white dark:bg-slate-800 rounded-lg shadow-lg border dark:border-slate-700
                            rtl:right-auto rtl:left-0" {{-- <-- AJOUT 3: Positionnement du drawer --}}
                     x-transition>
                    <a href="#" class="block px-4 py-2 rounded-lg  hover:bg-slate-100 dark:hover:bg-slate-700">Mon profil</a>
                   
                    <form method="POST" action="{{ tenant() ? route('logout') : route('logoutadmin')  }}">
                                    @csrf
                                    <button type="submit" class="block px-4 py-2 rounded-lg  w-full hover:bg-slate-100 dark:hover:bg-slate-700">
